...
(function (self) {
	...
	*/
	...
	self.calculateSales = function (company) {
		...
		var bw = Camelot2.Grid.Interface.budgetWeightings(); // nc mark main
		var ex = Camelot2.Store().gridExpend; // Grid expenditure cash
		...
		potInc += 5 * ((fans * 0.05) * markPer);
		potInc *= (1 - mainPer); // New Content sales. + Marketing Mod - Maintenence Mod

		var lastWeek = Camelot2.Store().lastWeekPotInc; // Get last weeks potential income
		...
		if (ex >= 0) {
			...
			Camelot2.Store().lastWeekPotInc = lastWeek;
		}

	};

	self.newContentMod = function () {
		var a = Camelot2.Store().ncMod;
		if (a > 0) { return a; }
		Camelot2.Store().ncMod = 1;
		return Camelot2.Store().ncMod;
	}
	...
	function calculateNewContent(nex) {
		...
		var nc = Math.floor(nex * (GameManager.company.gameLog.length / 10000)); // New Content amount NCSpending * ((amount of games already made) / 1000)
	...
	function totalRevenue() {
		...
		for (var i = 0; i < GameManager.company.gameLog.length; i++) {
			totRev += GameManager.company.gameLog[i].revenue; // Add up revenue of all games.
		}
		return totRev;
	};
